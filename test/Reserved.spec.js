...
tape(`@before()`, function(test){
	...
	test.deepEqual(Filler.tree, {total: 3}, "should be called before a data row enters the loop") 
...
tape(`@after()`, function(test){
  ...
  test.deepEqual(Filler.tree, {total: 0}, "should not be applied to the result")
  test.deepEqual(
  	opts.data, 
...
tape(`@ignore()`, function(test){
	...
	test.equal(
		Filler.tree.byCount,
		6, 
		"should filter if value is in array"
	)
	test.deepEqual(
